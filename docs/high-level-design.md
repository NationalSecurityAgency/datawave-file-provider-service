...
- **An observable audit trail** — every context change is recorded in a git-backed (SPI-driven) ledger and exposed via the REST API.
...
6. Record every mutating operation in an SPI-driven audit trail (git-backed by default) and expose it via API.
7. Operate as a standard Datawave microservice (starter security, config server, Consul discovery).
...
- **A database.** State lives in HDFS (files, versions, manifests), ZooKeeper (pointer), and git (audit). The service holds only caches rebuilt from those.
...
- **Garbage collection of old versions** — each version directory is a self-contained full copy in HDFS (which keeps future retention/GC trivial), and version churn costs tservers nothing (§1.1 cache keying); a retention policy can be added later.
...
flowchart LR
    ...
    subgraph svc [Context Service]
        ...
        AUD[Audit SPI<br/>default: git]
    end

    subgraph storage [Data plane]
        HDFS[(HDFS<br/>staging / versions / current)]
        ZK[(ZooKeeper)]
        GIT[(Git repo)]
    end
...
| **Audit SPI** | `AuditTrailStore` interface; default `GitAuditTrailStore` (JGit). Append-only ledger of context events. |

New dependencies beyond the Datawave starter: `hadoop-client` (HDFS), `curator-framework`/`curator-recipes` (the starter has no ZooKeeper/Curator utility to reuse — only Accumulo's own client wiring), `org.eclipse.jgit`.
...
6. Staged files that made it into the version are cleared from `staging/` — matched by name **and** the checksum captured at assembly time, so a file re-uploaded mid-publish is left in staging as pending rather than silently destroyed. Every step is audited.
...
`ContextAuditEvent`: timestamp, context, action (`UPLOAD`, `DETECT`, `REMOVE`, `PUBLISH`, `ACTIVATE`, `RECONCILE`), principal (from the JWT proxy chain, or `hdfs-autodetect`), version, files affected (name/size/checksum), free-form detail.

**Default: git** (`file-provider.audit.type=git`), via JGit on a local repository (`file-provider.audit.git.path`), one JSON-lines file per context (`<context>.jsonl`), one commit per event with a human-readable commit message (`activate analytics 20260811T1432Z by cn=deployer,...`). Rationale, per guidance:

- No database; git is a component already available everywhere in this ecosystem.
- Independently observable: `git log`, `git show`, and diffs give an operator the full history with standard tooling, even if the service is down.
- Tamper-evident by construction (hash-chained commits).
- Optional mirror to a remote (`git push` after each commit) is a config flag, not a design change.

**Durability requirement:** the audit trail is a first-class deliverable, so `file-provider.audit.git.path` MUST be persistent storage (a persistent volume in containerized deployments) **or** the remote mirror MUST be enabled — an ephemeral local repo with no mirror is a misconfiguration the service refuses at startup. If the repo is missing/corrupt at startup, the service re-initializes it (cloning from the mirror when configured) and records an audited `RECONCILE` event noting any history loss.

The API (`GET .../audit`) reads through the same SPI, so the git repo layout is an implementation detail. Note this audit trail is a *context-change ledger*, deliberately distinct from Datawave's `spring-boot-starter-datawave-audit` / `AuditClient`, which is query-execution-shaped and requires the audit microservice; it was evaluated and does not fit file-lifecycle events. If a deployment later wants those events forwarded there too, that is one more `AuditTrailStore` implementation.
...
- **Health/observability:** actuator health indicators for HDFS reachability, ZooKeeper connectivity, and audit-repo writability; DropWizard metrics via the starter for upload/publish counts and durations.
...
- **Version retention/GC**, quiescence-based auto-publish, Spring Cloud Bus change notifications, per-file supersede rules richer than name-based (e.g., date-stamped logical names).
...
| Audit store | SPI, git repo default (JGit; persistent path or mandatory mirror) | Guidance: no DB, already-available component, observable with standard tooling, API-exposed |
| State/database | None — HDFS + ZK + git only | YAGNI; service is rebuildable from its stores |
